...
export default function SalaryCalculator() {
  ...
  const [paidLeaveHours, setPaidLeaveHours] = useState(null);
  const [paidLeaveMinutes, setPaidLeaveMinutes] = useState(null);
  const [extraLeaveDays, setExtraLeaveDays] = useState(null);
  const [extraLeaveHours, setExtraLeaveHours] = useState(null);
  const [extraLeaveMinutes, setExtraLeaveMinutes] = useState(null);
  ...
  /* NEW: Total Deduction (PF, ESIC, Penalty, etc..) */
  ...
  useEffect(() => {
    const h = (e) => {
      ...
      Object.keys(otCalRefs.current).forEach((idx) => {
        if (
          otCalRefs.current[idx] &&
          !otCalRefs.current[idx].contains(e.target)
        )
  ...
  const sundaysInMonth = (y, m) => {
    let s = 0;
    for (let d = 1; d <= daysInMonth(y, m); d++)
      if (new Date(y, m, d).getDay() === 0) s++;
    return s;
  };
  const workingDays = (s, e) => {
    let w = 0,
      c = new Date(s);
    while (c <= e) {
      if (c.getDay() !== 0) w++;
  ...
  const propSalary = (s, e, base) => {
    if (s.getMonth() === e.getMonth() && s.getFullYear() === e.getFullYear()) {
      const td = daysInMonth(s.getFullYear(), s.getMonth()),
        sun = sundaysInMonth(s.getFullYear(), s.getMonth());
    ...
    while (cur <= e) {
      ...
      const ms = new Date(y, m, 1),
        me = new Date(y, m + 1, 0);
      const rs = cur > ms ? cur : ms,
        re = e < me ? e : me;
      total +=
        (base / (daysInMonth(y, m) - sundaysInMonth(y, m))) *
        workingDays(rs, re);
  ...
  const updateOt = (i, f, v) => {
    ...
    else if (["hours", "minutes", "seconds"].includes(f)) {
      u[i].otMode = "time";
      u[i].otPercent = "";
    }
    setOtEntries(u);
  ...
  const pct2time = (p) => {
    const m = parseFloat(p) * 9 * 60;
    return {
      h: Math.floor(m / 60),
      m: Math.floor(m % 60),
      s: Math.round((m * 60) % 60),
      totalMins: m,
    };
  };
  const otMins = (e) =>
    e.otMode === "percent" && e.otPercent !== ""
      ? parseFloat(e.otPercent) * 9 * 60
      : (parseInt(e.hours) || 0) * 60 +
        (parseInt(e.minutes) || 0) +
        (parseInt(e.seconds) || 0) / 60;
  ...
  const calculate = () => {
    ...
    const otBD = validOT.map((x) => {
      const m = otMins(x),
        p = otPct(m),
        w = m * pmrOT;
      return {
        date: x.date,
        hours: parseInt(x.hours) || 0,
        minutes: parseInt(x.minutes) || 0,
        seconds: parseInt(x.seconds) || 0,
        totalMinutes: m,
        percent: p,
        wage: w,
        mode: x.otMode,
    ...
    const dedAmt = parseFloat(totalDeduction) || 0;
    ...
    if (isThisMonth || isFullMonth) {
      ...
      if (calculationMode === "day") {
        ...
        if (usedExtra) {
          ud = 2;
          extraM = (parseInt(extraDays) || 0) * mpd;
        } else if (usedTwoPaid) ud = 2;
        else if (usedOnePaid) ud = 1;
        usedPM = ud * mpd;
        unusedPD = 2 - ud;
      } else {
        const pl =
          (parseInt(paidLeaveDays) || 0) * mpd +
          (parseInt(paidLeaveHours) || 0) * 60 +
          (parseInt(paidLeaveMinutes) || 0);
        const el =
          (parseInt(extraLeaveDays) || 0) * mpd +
          (parseInt(extraLeaveHours) || 0) * 60 +
          (parseInt(extraLeaveMinutes) || 0);
        if (pl > 2 * mpd) {
          alert("Paid leave cannot exceed 2 days");
          return;
        }
        usedPM = pl;
        unusedPD = Math.floor((2 * mpd - pl) / mpd);
        extraM = el;
      }
      const effM = bm + unusedPD * mpd - extraM;
      final = pmr * effM;
      setResult({
        baseSalary: base,
        baseWorkingDays: wd,
        baseMinutes: bm,
        usedPaidMinutes: usedPM,
        extraMinutes: extraM,
        unusedPaidDays: unusedPD,
        inHand: Math.round(final),
        calculationMode,
        totalDays,
        sundays: suns,
        perMinuteRate: pmr.toFixed(4),
        perHourRate: (base / wd / 9).toFixed(2),
        perDayRate: (base / wd).toFixed(2),
        effectiveMinutes: Math.round(effM),
        monthName: isThisMonth ? curMonthName() : getMonthName(s),
        isCustomRange: !isThisMonth,
        isFullMonth: true,
        proportionalBase: Math.round(base),
        deduction: Math.round(base - final),
        totalOtMinutes: totOTm,
        totalOtPercent: totOTpct,
        totalOtAmount: totOTamt,
        otBreakdown: otBD,
        otherDeduction: dedAmt,
        netSalary: Math.round(final + totOTamt - dedAmt),
      });
    } else {
      const ps = propSalary(s, e, base);
      let ded = 0;
      const dr = base / wd;
      if (calculationMode === "day") {
        let pld = 0;
        if (usedExtra) {
          pld = 2;
          ded += (parseInt(extraDays) || 0) * dr;
        } else if (usedTwoPaid) pld = 2;
        else if (usedOnePaid) pld = 1;
        const apd =
          2 *
          (wd /
            (daysInMonth(s.getFullYear(), s.getMonth()) -
              sundaysInMonth(s.getFullYear(), s.getMonth())));
      ...
      setResult({
        ...
        unusedPaidDays:
          calculationMode === "day"
            ...
            : Math.max(
                0,
                2 -
                  ((parseInt(paidLeaveDays) || 0) +
                    ((parseInt(paidLeaveHours) || 0) +
                      (parseInt(paidLeaveMinutes) || 0) / 60) /
                      9),
              ),
        inHand: Math.round(final),
        calculationMode,
        totalDays,
        sundays: suns,
        perMinuteRate: (base / (wd * mpd)).toFixed(4),
        perHourRate: (base / wd / 9).toFixed(2),
        perDayRate: (base / wd).toFixed(2),
        effectiveMinutes: Math.round(wd * mpd),
        monthName: fmtRange(s, e),
        isCustomRange: true,
        isFullMonth: false,
        proportionalBase: Math.round(ps),
        deduction: Math.round(ded),
        totalOtMinutes: totOTm,
        totalOtPercent: totOTpct,
        totalOtAmount: totOTamt,
        otBreakdown: otBD,
  ...
  const fmtRange = (s, e) =>
    !s || !e
      ? "--"
      : `${s.toLocaleDateString("en-GB")} to ${e.toLocaleDateString("en-GB")}`;
  const periodDisplay = () => {
    if (isThisMonth) return curMonthName();
    if (startDate && endDate)
      return fmtRange(new Date(startDate), new Date(endDate));
    return "--";
  };
  ...
  const Row = ({ label, value, color, bold, last }) => (
    <div
      style={{
        ...
        alignItems: "center",
        padding: "5px 0",
        borderBottom: last ? "none" : `1px dashed ${C.borderDk}`,
      }}
    >
      <span
        style={{
  ...
  const rtdRight = { ...rtd, textAlign: "right", fontWeight: 600 };
  const rth = {
    padding: "3px 2px",
  ...
  const printReceipt = () => {
    ...
    if (!printWindow) {
      alert(
        "Please allow pop-ups for this site so the receipt can open in a new tab.",
      );
      return;
    }
    ...
    <>
      <style>{`
        ...
        * { box-sizing:border-box; }
        ...
        /* OT + Total Deduction: side-by-side → stack on mobile */
        .ot-deduct-grid {
          display: grid;
        ...
        <div style={{ maxWidth: "1440px", margin: "0 auto" }}>
              ...
              <div>
                <h1
                  style={{
                    ...
                    textShadow: dm
                      ? "0 0 20px rgba(200,200,180,0.15)"
          ...
          <div className="main-grid">
            ...
            >
                ...
                <div style={{ ...panel }}>
                  ...
                  <div
                    style={{
                      marginTop: "8px",
                      fontSize: "14px",
                      color: isFullMonth ? C.green : "#b06020",
                      letterSpacing: "0.04em",
                    }}
                  >
                    {isFullMonth ? (
                  ...
                  <div style={{ position: "relative" }} ref={startCalRef}>
                    <button
                      ...
                      style={{
                        ...
                        cursor: isThisMonth ? "not-allowed" : "pointer",
                      }}
                    >
                      <span>
                        {startDate ? fmtDisplay(startDate) : "Pick date"}
                      </span>
                    ...
                    {[
                      ...
                      <label key={v} className="rradio">
                        <input
                          ...
                          name="mode"
                          value={v}
                          checked={calculationMode === v}
                          onChange={() => {
                            setCalculationMode(v);
                            v === "day" ? resetTime() : resetDay();
                          }}
                        />
                        <div className="rrdot">
                          <div className="rrfill" />
                        </div>
                          ...
                          {[
                            ...
                            <label key={item.type} className="rcheck">
                              <input
                                ...
                                onChange={() => handleDayChk(item.type)}
                              />
                              <div className="rchkbox">
                                <div className="rchktick" />
                              </div>
                              <span style={{ fontSize: "16px", color: C.text }}>
                                {item.label}
                              </span>
                        ...
                        <div className="leave-time-grid">
                          {[
                            ...
                            },
                            {
                              ...
                              maxes: [undefined, "23", "59"],
                            },
                          ].map((g, gIdx) => (
                            <React.Fragment key={g.title}>
                              {gIdx === 1 && (
                                <div
                                  className="leave-divider"
                                  style={{
                                    display: "flex",
                                    alignItems: "stretch",
              ...
              {/* ROW C: OT Section + Total Deduction — side by side */}
              <div className="ot-deduct-grid">
                {/* Overtime panel */}
                <div style={panel}>
                  <div
                    style={{
                  ...
                  >
                    {otEntries.map((entry, idx) => {
                      const isP =
                        entry.otMode === "percent" && entry.otPercent !== "";
                      const hasT =
                        parseInt(entry.hours) > 0 ||
                        parseInt(entry.minutes) > 0 ||
                        parseInt(entry.seconds) > 0;
                      const rev = isP ? pct2time(entry.otPercent) : null;
                      const mins = otMins(entry),
                        pct = otPct(mins);
                      ...
                      return (
                        ...
                        >
                          ...
                          <div style={{ marginBottom: "7px" }}>
                            <label style={{ ...lbl, fontSize: "16px" }}>
                              Date
                            </label>
                            <div
                              style={{ position: "relative" }}
                              ref={(el) => {
                                otCalRefs.current[idx] = el;
                              }}
                            >
                              <button
                                type="button"
                          ...
                          {isP && rev && (
                            ...
                            >
                              {entry.otPercent}% → {rev.h}h {rev.m}m {rev.s}s |{" "}
                              {rev.totalMins.toFixed(1)} min
                            </div>
                          )}
                          {!isP && hasT && (
                            <div
                            ...
                            >
                              ...
                              {entry.seconds || 0}s →{" "}
                              <strong>{pct.toFixed(3)}% OT</strong>
                            </div>
                          )}
                        </div>
                      );
                ...
                {/* NEW: Total Deduction panel */}
                <div style={panel}>
                  <div style={secHead}>Total Deduction</div>
                  <p
                    style={{
                      margin: "0 0 10px 0",
                      fontSize: "13px",
                  ...
                  <label style={{ ...lbl, fontSize: "16px" }}>Amount (₹)</label>
                  <input
                    type="number"
            ...
            >
              ...
              >
                ...
                <div
                  style={{
                    ...
                    color: C.displayTxt,
                    textShadow: result
                      ? `0 0 10px ${C.displayTxt},0 0 28px rgba(100,255,120,0.25)`
                      : "none",
                  ...
                  <>
                    ...
                    >
                      <span>
                        ...
                        />
                        Your Basic Salary: ₹{result.inHand.toLocaleString()}
                      </span>
                      {result.otBreakdown?.length > 0 && (
                        <>
                          ...
                          <span>
                            ...
                            />
                            Your Total OT: ₹
                            {Math.round(result.totalOtAmount).toLocaleString()}
                          </span>
                        </>
                      )}
                      {result.otherDeduction > 0 && (
                        <>
                          ...
                          <span>
                            Deduction: ₹
                            {Math.round(result.otherDeduction).toLocaleString()}
                          </span>
                        </>
                      )}
                    </div>
              ...
              <div className="result-grid">
                ...
                <div style={panel}>
                  ...
                  <Row
                    label="Total"
                    value={result ? `${result.totalDays}d` : "--"}
                  />
                  <Row
                    label="Sundays"
                    value={result ? `${result.sundays}d` : "--"}
                  />
                  <Row
                    label="Working"
                    value={result ? `${result.baseWorkingDays}d` : "--"}
                    bold
                    last
                  />
                </div>
                ...
                <div style={panel}>
                  ...
                  <Row
                    label="Base"
                    value={
                      result ? `₹${result.baseSalary.toLocaleString()}` : "--"
                    }
                  />
                  <Row
                    label="Base Mins"
                    value={
                      result ? `${result.baseMinutes.toLocaleString()}` : "--"
                    }
                  />
                  <Row
                    label="₹/min"
                  ...
                  <Row
                    label="₹/hour"
                    value={result ? `₹${result.perHourRate}` : "--"}
                  />
                  <Row
                    label="₹/day"
                    value={result ? `₹${result.perDayRate}` : "--"}
                  />
                  {result?.isCustomRange && !result?.isFullMonth && (
                    <>
                      <Row
                        ...
                        value={
                          result
                            ? `₹${result.proportionalBase.toLocaleString()}`
                            : "--"
                        }
                      />
                      <Row
                        ...
                        value={
                          result
                            ? `-₹${result.deduction.toLocaleString()}`
                            : "--"
                        }
                        color={C.red}
                      />
                  ...
                  {result?.otherDeduction > 0 && (
                    <Row
                      label="Other Ded."
                      value={`-₹${Math.round(result.otherDeduction).toLocaleString()}`}
                      color={C.red}
                    />
                  )}
                  <Row
                    label="In-Hand"
                    value={result ? `₹${result.inHand.toLocaleString()}` : "--"}
                    color={C.displayTxt}
                    bold
                    last
                  />
                ...
                {(isThisMonth || isFullMonth) && (
                  <div style={panel}>
                    ...
                    <Row
                      label="Paid Used"
                      value={result ? `${result.usedPaidMinutes}m` : "--"}
                    />
                    <Row
                      label="Extra"
                      value={result ? `-${result.extraMinutes}m` : "--"}
                      color={C.red}
                    />
                    <Row
                      label="Unused PL"
                      value={result ? `+${result.unusedPaidDays}d` : "--"}
                      color={C.green}
                    />
                    <Row
                      label="Mode"
                ...
                <div style={panel}>
                  ...
                  <Row
                    label="OT Minutes"
                    value={
                      result ? `${result.totalOtMinutes.toFixed(1)}m` : "--"
                    }
                  />
                  <Row
                    label="OT Hours"
                    value={
                      result
                        ? `${(result.totalOtMinutes / 60).toFixed(2)}h`
                        : "--"
                    }
                  />
                  <Row
                    label="OT %"
                    value={
                      result ? `${result.totalOtPercent.toFixed(3)}%` : "--"
                    }
                  />
                  <Row
                    label="OT Amount"
                    ...
                    {result.otBreakdown.map((ot, i) => (
                      ...
                      >
                        <div>
                          ...
                          >
                            {new Date(ot.date + "T00:00:00").toLocaleDateString(
                              "en-IN",
                              {
                                day: "2-digit",
                                month: "short",
                          ...
                          >
                            ...
                            />
                            {Math.floor(ot.totalMinutes / 60)}h{" "}
                            {Math.floor(ot.totalMinutes % 60)}m
                          </span>
                        </div>
                        <div
                          style={{
                        ...
                        >
                          ...
                          >
                            ₹{Math.round(ot.wage).toLocaleString()}
                          </span>
                        </div>
                      </div>
                    ))}
              ...
              >
                <FiPrinter size={18} /> PRINT / DOWNLOAD RECEIPT (PDF)
              </button>
            </div>
          </div>
          ...
          >
            ● BUILT BY KRISH ● SALARY CALCULATOR ● *Terms of{" "}
            {new Date().getFullYear()} Used ●
          </div>
        </div>
      </div>

      {/* Printable Receipt — invisible on screen, shown only when printing / saving as PDF */}
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: "11px", marginTop: "2px" }}>
              Pay Period: {result?.monthName || periodDisplay()}
            </div>
          ...
          {result ? (
            <>
              <div style={rSecTitle}>Salary Details</div>
              <table
                style={{
                  width: "100%",
              ...
              >
                <tbody>
                  <tr>
                    <td style={rtd}>Calculation Type</td>
                    <td style={rtdRight}>
                      {result.isFullMonth
                        ? "Full Month"
                  ...
                  <tr>
                    <td style={rtd}>Total Days</td>
                    <td style={rtdRight}>{result.totalDays}</td>
                  </tr>
                  <tr>
                    <td style={rtd}>Sundays</td>
                    <td style={rtdRight}>{result.sundays}</td>
                  </tr>
                  <tr>
                    <td style={rtd}>Working Days</td>
                    <td style={rtdRight}>{result.baseWorkingDays}</td>
                  </tr>
                  <tr>
                    <td style={rtd}>Base Monthly Salary</td>
                    <td style={rtdRight}>
                      ₹{result.baseSalary.toLocaleString()}
                    </td>
                  </tr>
                  <tr>
                    <td style={rtd}>Per Minute Rate</td>
                    <td style={rtdRight}>₹{result.perMinuteRate}</td>
                  </tr>
                  <tr>
                    <td style={rtd}>Per Hour Rate</td>
                    <td style={rtdRight}>₹{result.perHourRate}</td>
                  </tr>
                  <tr>
                    <td style={rtd}>Per Day Rate</td>
                    <td style={rtdRight}>₹{result.perDayRate}</td>
                  </tr>
                  {result.isCustomRange && !result.isFullMonth && (
                    <tr>
                      <td style={rtd}>Proportional Base</td>
                      <td style={rtdRight}>
                        ₹{result.proportionalBase.toLocaleString()}
                      </td>
                    </tr>
              ...
              <div style={rSecTitle}>Leave Details</div>
              <table
                style={{
                  width: "100%",
                  borderCollapse: "collapse",
              ...
              >
                <tbody>
                  <tr>
                    <td style={rtd}>Calculation Mode</td>
                    <td style={rtdRight}>{result.calculationMode}-based</td>
                  </tr>
                  <tr>
                    <td style={rtd}>Paid Leave Used</td>
                    <td style={rtdRight}>{result.usedPaidMinutes} min</td>
                  </tr>
                  <tr>
                    <td style={rtd}>Extra Leave</td>
                    <td style={rtdRight}>-{result.extraMinutes} min</td>
                  </tr>
                  <tr>
                    <td style={rtd}>Unused Paid Leave</td>
                    <td style={rtdRight}>+{result.unusedPaidDays} day(s)</td>
                  </tr>
                  {result.deduction >= 0 ? (
                    <tr>
                      <td style={rtd}>Deduction (leave-based)</td>
                      <td style={rtdRight}>
                        -₹{result.deduction.toLocaleString()}
                      </td>
                    </tr>
                    ...
                    <tr>
                      <td style={rtd}>Leave Credit (extra unused leave)</td>
                      <td style={rtdRight}>
                        +₹{Math.abs(result.deduction).toLocaleString()}
                      </td>
                    </tr>
                  )}
                </tbody>
              ...
              {result.otBreakdown?.length > 0 && (
                <>
                  <div style={rSecTitle}>Overtime Breakdown</div>
                  <table
                    style={{
                      width: "100%",
                      borderCollapse: "collapse",
                  ...
                  >
                    <thead>
                      <tr>
                        ...
                        <th style={rth}>Duration</th>
                        <th style={{ ...rth, textAlign: "right" }}>OT %</th>
                        <th style={{ ...rth, textAlign: "right" }}>Amount</th>
                      </tr>
                    </thead>
                    <tbody>
                      {result.otBreakdown.map((ot, i) => (
                        <tr key={i}>
                          ...
                          <td style={rtdRight}>{ot.percent.toFixed(3)}%</td>
                          <td style={rtdRight}>
                            ₹{Math.round(ot.wage).toLocaleString()}
                          </td>
                        </tr>
                      ...
                      <tr>
                        <td style={{ ...rtd, fontWeight: 700 }}>Total OT</td>
                        <td style={rtd}>
                          {result.totalOtMinutes.toFixed(1)} min
                        </td>
                        <td style={{ ...rtdRight, fontWeight: 700 }}>
                          {result.totalOtPercent.toFixed(3)}%
                        </td>
                        <td style={{ ...rtdRight, fontWeight: 700 }}>
                          ₹{Math.round(result.totalOtAmount).toLocaleString()}
                        </td>
              ...
              <div style={rSecTitle}>Final Settlement</div>
              <table
                style={{
                  width: "100%",
                  borderCollapse: "collapse",
              ...
              >
                <tbody>
                  <tr>
                    <td style={rtd}>In-Hand (Base after leave)</td>
                    <td style={rtdRight}>₹{result.inHand.toLocaleString()}</td>
                  </tr>
                  <tr>
                    <td style={rtd}>Overtime Amount</td>
                    <td style={rtdRight}>
                      +₹{Math.round(result.totalOtAmount).toLocaleString()}
                    </td>
                  </tr>
                  {result.otherDeduction > 0 && (
                    <tr>
                      <td style={rtd}>
                        Other Deduction (PF, ESIC, Penalty, etc..)
                      </td>
                      <td style={rtdRight}>
                        -₹{Math.round(result.otherDeduction).toLocaleString()}
                      </td>
            ...
            >
              No calculation available. Please calculate salary before printing.
            </div>
          )}
          ...
          >
            This is a computer-generated salary receipt. ● Salary Calculator ●{" "}
