...
function SalaryCalculator() {
  ...
  // Set default dates for current month - FIXED VERSION
  useEffect(() => {
    if (isThisMonth) {
      const now = new Date();
  ...
  const calculateSalary = () => {
    ...
    if (isThisMonth) {
      // FIXED: Use current month correctly
    ...
    if (!baseSalary || baseSalary <= 0 || baseWorkingDays <= 0) {
  ...
  // FIXED: Proper month name display
  ...
  return (
    <div className={`min-h-screen ${theme.background} py-8 px-4 transition-colors duration-300 [font-family:"verdana"]`}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-8">
          <div className="flex justify-between items-center mb-6">
            ...
            <button
              ...
              className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ${
                darkMode ? 'bg-cyan-600 text-white' : 'bg-cyan-100 text-cyan-600'
              }`}
            >
              {darkMode ? '☀️' : '🌙'}
            </button>
          ...
          {isThisMonth && (
            <div className={`mt-2 inline-block px-3 py-1 rounded-full text-sm ${
              ...
              Currently calculating for: <strong>{getCurrentMonthName()}</strong>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Input Form */}
          <div className={`rounded-2xl shadow-xl p-6 border-2 ${theme.card} transition-all duration-300`}>
            {/* Month Toggle */}
            <div className={`mb-6 p-4 rounded-xl border ${theme.section}`}>
              <div className="flex items-center justify-between">
                <span className={`font-semibold ${theme.text}`}>Calculation Period</span>
                <label className="relative inline-flex items-center cursor-pointer">
                  ...
                  } peer-focus:ring-4 peer-focus:ring-cyan-300 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[4px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-6 after:w-6 after:transition-all peer-checked:bg-cyan-600`}></div>
                  <span className={`ml-3 text-sm font-medium ${theme.text}`}>
              ...
              {isThisMonth && (
                <p className={`text-sm mt-2 ${theme.textSecondary}`}>
                  Calculating for <span className="text-cyan-600 font-semibold">{getCurrentMonthName()}</span>
                </p>
              )}
            </div>
            ...
            <div className={`mb-6 p-5 rounded-xl border ${theme.section}`}>
              <p className={`text-sm font-semibold mb-3 ${theme.text}`}>Leave Input Mode</p>
            ...
            {/* Day-based Inputs */}
            {calculationMode === 'day' && (
              <div className={`mb-6 space-y-4 p-5 rounded-xl border ${theme.section}`}>
                <label className="flex items-center space-x-3 cursor-pointer group">
                  ...
                  />
                  <span className={`group-hover:text-cyan-600 transition-colors ${theme.text}`}>
                    Used 1 paid leave
                  </span>
                </label>
            ...
            {calculationMode === 'time' && (
              <div className={`mb-6 space-y-6 p-5 rounded-xl border ${theme.section}`}>
                <div>
                  <p className={`text-sm font-semibold mb-3 ${theme.text}`}>
                    Paid Leave Used <span className="text-cyan-600">(max 2 days)</span>
                  </p>
                  <div className="grid grid-cols-3 gap-3">
                    {['Days', 'Hours', 'Minutes'].map((label, index) => (
                      <div key={label}>
                        <label className={`block text-xs font-medium mb-1 text-center ${theme.textSecondary}`}>
                          {label}
                        </label>
                        <input
                          type="number"
                          value={[paidLeaveDays, paidLeaveHours, paidLeaveMinutes][index]}
                          onChange={(e) => {
                            const setters = [setPaidLeaveDays, setPaidLeaveHours, setPaidLeaveMinutes];
                            setters[index](e.target.value);
                          }}
                          min="0"
                          max={label === 'Days' ? '2' : label === 'Hours' ? '23' : '59'}
                          placeholder="0"
                ...
                <div>
                  <p className={`text-sm font-semibold mb-3 ${theme.text}`}>Extra Leave Taken</p>
                  <div className="grid grid-cols-3 gap-3">
                    {['Days', 'Hours', 'Minutes'].map((label, index) => (
                      <div key={label}>
                        <label className={`block text-xs font-medium mb-1 text-center ${theme.textSecondary}`}>
                        ...
                        <input
                          type="number"
                          value={[extraLeaveDays, extraLeaveHours, extraLeaveMinutes][index]}
                          onChange={(e) => {
                            const setters = [setExtraLeaveDays, setExtraLeaveHours, setExtraLeaveMinutes];
                            setters[index](e.target.value);
                          }}
                          min="0"
          ...
          <div className={`rounded-2xl shadow-xl p-6 border-2 ${theme.result} transition-all duration-300`}>
            ...
            <div className="space-y-6">
              ...
              <div className={`p-4 rounded-xl border ${theme.section}`}>
                <h3 className={`text-lg font-semibold mb-3 ${theme.text} border-b pb-2`}>
                  Month Information
                </h3>
                <div className="space-y-2">
                  {[
                    { label: 'Month', value: getPeriodDisplay() },
                    { label: 'Total Days', value: result ? `${result.totalDays} days` : '--' },
                    { label: 'Sundays', value: result ? `${result.sundays} days` : '--' },
                    { label: 'Working Days', value: result ? `${result.baseWorkingDays} days` : '--' }
                  ].map((item, index) => (
                ...
                <div className="space-y-2">
                  {[
                    { 
                      ...
                      value: result ? `${result.usedPaidMinutes} minutes` : '--',
                      color: theme.text
                    },
                    { 
                      label: 'Extra Leave', 
              ...
              <div className={`p-4 rounded-xl border ${theme.section}`}>
                ...
                <div className="space-y-2">
                  {[
                    { label: 'Base Salary', value: result ? `₹${result.baseSalary.toLocaleString()}` : '--' },
                    { label: 'Base Minutes', value: result ? `${result.baseMinutes.toLocaleString()} min` : '--' },
                    { label: 'Paid per Minute', value: result ? `₹${result.perMinuteRate}` : '--' },
                    { label: 'Your Minutes', value: result ? `${result.effectiveMinutes.toLocaleString()} min` : '--' }
                  ].map((item, index) => (
                    <div key={index} className="flex justify-between">
                      <span className={theme.textSecondary}>{item.label}:</span>
                      <span className={`font-medium ${theme.text}`}>{item.value}</span>
              ...
              <div className="text-center p-6 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-xl border border-cyan-400">
                <p className="text-white/80 text-sm mb-2">Your In-Hand Salary Will Be</p>
        ...
        <div className={`text-center mt-8 text-sm ${theme.textSecondary}`}>
          <p>Built by Krish with React.js • Professional IT Salary Calculator • *Terms of {new Date().getFullYear()} Used</p>
        </div>
      </div>
    </div>
  );
